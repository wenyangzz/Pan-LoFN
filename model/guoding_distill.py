...
class GPPNNPro(nn.Module):
    def __init__(self,
                 ms_channels,
                 pan_channels,
                 n_feat,
                 n_layer=6):
        super(GPPNNPro, self).__init__()
...
class Net(nn.Module):
    ...
    def forward(self, ms, b_ms, pan=None):
        ...
        fold1 = self.unfold1(ms_en,ms,pan)[0]+ms_en
        listunfold = self.unfold2(fold1,ms,pan)
        fold2 =listunfold[0]+fold1

        return fold2+mHR, finput, fmid, listunfold[1], listunfold[2]
...
class LRMS(nn.Module):
    ...
    def forward(self, HR, LR, PAN):
        ...
        return HR, HR_Residual_1, HR_Residual_2
