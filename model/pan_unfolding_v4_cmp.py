...
class pan_unfolding(nn.Module):
    def __init__(self, mid_channels=64, T = 4):
        ...
        print("now: pan_unfolding_V4_cmp")
        self.up_factor = 4
        G0 = mid_channels
        kSize = 3
        ...
        self.conv_up = nn.ModuleList([nn.Sequential(*[
            Conv_up(4, G0, self.up_factor)
        ]) for i in range(T)])
        self.conv_down = nn.ModuleList([nn.Sequential(*[
            Conv_down(4, G0, self.up_factor)
        ]) for i in range(T)])

        self.rm1 = nn.ModuleList([nn.Sequential(*[
            att_spatial(res_num=3)
        ]) for i in range(T)])

        self.NLBlock = nn.ModuleList([nn.Sequential(*[
            blockNL(4, 15)
        ]) for i in range(T)])
    ...
    def forward(self, lms, b_ms, pan):
        ...
        for i in range(len(self.conv_u)):
            ...
            # denoising module
            rm1_s2_0 = pan_hp + self.rm1[i](torch.cat([torch.unsqueeze(uk[:,0,:,:],1), pan], 1)) * pan_hp  # B 1 256 256
            rm1_s2_1 = pan_hp + self.rm1[i](torch.cat([torch.unsqueeze(uk[:,1,:,:],1), pan], 1)) * pan_hp  # B 1 256 256
            rm1_s2_2 = pan_hp + self.rm1[i](torch.cat([torch.unsqueeze(uk[:,2,:,:],1), pan], 1)) * pan_hp  # B 1 256 256
            rm1_s2_3 = pan_hp + self.rm1[i](torch.cat([torch.unsqueeze(uk[:,3,:,:],1), pan], 1)) * pan_hp  # B 1 256 256
            ...
            # NARM
            NL = self.NLBlock[i](x)        # B 4 256 256
            if i!=0:
                vk = self.conv_u[i](torch.cat(vk_list+[NL], 1))
            else:
                vk = self.conv_u[i](NL)         # B 4 256 256
            # denoising module
            rm2_s2_0 = pan_hp + self.rm1[i](torch.cat([torch.unsqueeze(vk[:,0,:,:],1), pan], 1)) * pan_hp  # B 1 256 256
            rm2_s2_1 = pan_hp + self.rm1[i](torch.cat([torch.unsqueeze(vk[:,1,:,:],1), pan], 1)) * pan_hp  # B 1 256 256
            rm2_s2_2 = pan_hp + self.rm1[i](torch.cat([torch.unsqueeze(vk[:,2,:,:],1), pan], 1)) * pan_hp  # B 1 256 256
            rm2_s2_3 = pan_hp + self.rm1[i](torch.cat([torch.unsqueeze(vk[:,3,:,:],1), pan], 1)) * pan_hp  # B 1 256 256
            ...
            # iteration
            x = x - self.delta[i]*(self.conv_up[i](self.conv_down[i](x)-lms+self.u[i]*(self.conv_down[i](NL)-lms))+self.eta[i]*(x-decode_u)+self.gama[i]*(NL-decode_v))
