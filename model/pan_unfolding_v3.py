...
class pan_unfolding(nn.Module):
    def __init__(self, mid_channels=64):
        ...
        self.conv_u = nn.ModuleList([nn.Sequential(*[                         
            ...
            nn.Conv2d(4*(i+1), 4, kSize, padding=(kSize - 1) // 2, stride=1),
        ]) for i in range(T)])
    ...
    def forward(self, lms, b_ms, pan):
        ...
        for i in range(len(self.conv_u)):
            uk_list.append(x)
            uk = self.conv_u[i](torch.cat(uk_list, 1))         # B 4 256 256
            # uk_list.append(uk)
            ...
            # decode_u_list.append(decode_u)


            # NARM
            NL = self.NLBlock(x)        # B 4 256 256
            vk_list.append(NL)
            vk = self.conv_v[i](torch.cat(vk_list, 1))         # B 4 256 256
            # denoising module
            rm2_s2_0 = pan_hp + self.rm2(torch.cat([torch.unsqueeze(vk[:,0,:,:],1), pan], 1)) * pan_hp  # B 1 256 256
            rm2_s2_1 = pan_hp + self.rm2(torch.cat([torch.unsqueeze(vk[:,1,:,:],1), pan], 1)) * pan_hp  # B 1 256 256
            rm2_s2_2 = pan_hp + self.rm2(torch.cat([torch.unsqueeze(vk[:,2,:,:],1), pan], 1)) * pan_hp  # B 1 256 256
            rm2_s2_3 = pan_hp + self.rm2(torch.cat([torch.unsqueeze(vk[:,3,:,:],1), pan], 1)) * pan_hp  # B 1 256 256
