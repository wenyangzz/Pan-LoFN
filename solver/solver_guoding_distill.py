...
class Solver(BaseSolver):
    def __init__(self, cfg):
        ...
        self.model_t = net(
                ...
                base_filter=64,
                args=self.cfg)
        #print([self.model.parameters(), self.model_t.parameters()])



        #self.optimizer = maek_optimizer(self.cfg['schedule']['optimizer'], cfg,[{"params": self.model.parameters()}, {"params": self.model_t.parameters()}])
    ...
    def train(self): 
        with tqdm(total=len(self.train_loader), miniters=1,
            ...
            for iteration, batch in enumerate(self.train_loader, 1):
                ...
                y, yi, ym = self.model(lms_image, bms_image, pan_image)
                yt, yti, ytm = self.model_t(ms_image, bms_image, pan_image)

                loss = self.loss(y, ms_image)+self.loss(yi, yti)+self.loss(ym, ytm)
    ...
    def eval(self):
        with tqdm(total=len(self.val_loader), miniters=1,
            ...
            for iteration, batch in enumerate(self.val_loader, 1):
                ...
                self.model.eval()
                with torch.no_grad():
                    y, _, _ = self.model(lms_image, bms_image, pan_image)
    ...
    def check_gpu(self):
        ...
        if self.cuda:
            ...
            self.model_t = self.model_t.cuda(self.gpu_ids[0])
            self.model_t = torch.nn.DataParallel(self.model_t, device_ids=self.gpu_ids)

            keys1 = torch.load('/ghome/fuxy/DPFN-master-WV2/checkpoint_knowledge_WV2/knowledge_4_1629437633/best.pth',
                               map_location=lambda storage, loc: storage)['net']
            print(keys1.keys())
            self.model.load_state_dict(keys1)

            keys = torch.load('/ghome/fuxy/DPFN-master-WV2/checkpoint_gt2gt_WV2/gt2gt_4_1629473559/best.pth',
                              map_location=lambda storage, loc: storage)['net']
            self.model_t.load_state_dict(keys)
