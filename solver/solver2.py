...
class Solver(BaseSolver):
    def __init__(self, cfg):
        ...
        self.log_name = self.cfg['algorithm'] + '_' + str(self.cfg['data']['upsacle']) + '_' + str(self.now_time) + "_" + str(self.cfg['stage']) + 'stage' # str(self.timestamp)
        # save log
    ...
    def train(self, epoch): 
        with tqdm(total=len(self.train_loader), miniters=1,
            ...
            for iteration, batch in enumerate(self.train_loader, 1):
                ...
                if epoch < 2000:
                    z, jac_loss = self.model(lms_image, ms_image, pan_image)
                    y, jac_loss2 = self.model(lms_image, ms_image, pan_image, rev=True)
                    loss = self.loss(y, ms_image) / (self.cfg['data']['batch_size'] * 2)+jac_loss
                else:
                    z, jac_loss = self.model(lms_image, ms_image, pan_image)
                    loss = jac_loss
                # loss = jac_loss
                ...
                t.set_postfix_str("Batch loss {:.4f}, jac_loss {:.4f}".format(loss.item(), jac_loss.item()))
                t.update()
    ...
    def eval(self):
        with tqdm(total=len(self.val_loader), miniters=1,
            ...
            for iteration, batch in enumerate(self.val_loader, 1):
                ...
                self.model.eval()
                with torch.no_grad():
                    y,jac_loss  = self.model(lms_image, bms_image, pan_image)
                    # y, u, v  = self.model(lms_image, bms_image, pan_image)
                    loss = self.loss(y, ms_image)+jac_loss
    ...
    def test(self):
        ...
        for batch in self.data_loader:
            ...
            t0 = time.time()
            with torch.no_grad():
                prediction,_ = self.model(lms_image, bms_image, pan_image, rev=True)
            t1 = time.time()
    ...
    def run(self):
        ...
        try:
            while self.epoch <= self.nEpochs:
                ...
                if self.epoch>0.8*self.nEpochs:
                    self.test()
