...
class Testsolver(BaseSolver):
    ...
    def test(self):
        ...
        for batch in self.data_loader:
            ...
            t0 = time.time()
            with torch.no_grad():
                # prediction, _, noise = self.model(lms_image, bms_image, pan_image)
                prediction, _, noise = self.model(lms_image, bms_image, pan_image)
            ...
            # for i, img in enumerate(save_out):
            #     img = save_out[i]
            #     self.save_img(img.cpu().data, name[0][0:-4]+'_{}_stage.tif'.format(i), mode='RGB')

            #print("===> Processing: %s || Timer: %.4f sec." % (name[0], (t1 - t0)))
            avg_time.append(t1 - t0)
            self.save_img(bms_image.cpu().data, name[0][0:-4]+'_bic.tif', mode='RGB')  # CMYK
            self.save_img(ms_image.cpu().data, name[0][0:-4]+'_gt.tif', mode='RGB')
            self.save_img(prediction.cpu().data, name[0][0:-4]+'.tif', mode='RGB')
            self.save_img(noise.cpu().data, name[0][0:-4]+'_noise.tif', mode='RGB')
        print("===> AVG Timer: %.4f sec." % (np.mean(avg_time)))
        print("The path of predict:", os.path.join(self.cfg['test']['save_dir'], self.cfg['test']['type']+ "_" + str(self.cfg['stage']) + 'stage', str(self.now_time)))
        print("image saved!")
        ...
        # print('best_no_ref_results', self.best_no_ref_results['deep_no   '])
    ...
    def save_checkpoint(self):
        super(Testsolver, self).save_checkpoint()
